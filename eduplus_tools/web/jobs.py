...
def build_job_bundle(job: Job) -> Path | None:
    ...
    bundle_root = DEFAULT_WEB_BUNDLE_ROOT
...
def _job_output_root(payload: dict[str, object], job_id: str) -> Path:
    ...
    if execution_mode == "local":
        return Path(output_value or str(DEFAULT_LOCAL_OUTPUT_ROOT))
    base_output = Path(output_value or str(DEFAULT_WEB_OUTPUT_ROOT))
